...
const DraggableRow = React.memo(({ item, index, zoomLevel, showBaseline, showCriticalPath, isConflicted, onEdit, onDelete, onAddLooseTask, globalRank, interactionMode, allData, startDate, onTaskClick }) => {
    ...
    const isCritical = showCriticalPath && (item.dependencies.length > 0 || item.finalDate.isAfter(item.originalFinalDate));
    const cursorClass = interactionMode === 'horizontal' ? 'cursor-ew-resize' : 'cursor-ns-resize';
    const rowStyle = isOver && interactionMode === 'vertical' ? 'bg-blue-50 border-blue-300 z-10' : '';

    return (
        <div
            ref={setDroppableRef}
            style={{ position: 'absolute', top, left: 0, width: '100%', height: ROW_HEIGHT, transition: isDragging ? 'none' : 'top 0.3s ease' }}
                ...
                <Popover content={
                    <div className="text-xs w-56">
                        ...
                        <div className="flex flex-col gap-2 mt-2">
                            <Button size="small" block icon={<ThunderboltOutlined />} className="text-orange-700 border-orange-300 bg-orange-50 hover:bg-orange-100 dark:bg-orange-900/30 dark:border-orange-800 dark:text-orange-400" onClick={() => onAddLooseTask(item)}>
                                Adicionar Tarefa Avulsa
                            </Button>
                            <div className="flex gap-2">
                                <Button size="small" block icon={<EditOutlined />} onClick={() => onEdit(item)}>Editar</Button>
                                <Button size="small" block danger icon={<DeleteOutlined />} onClick={() => onDelete(item.id)}>Excluir</Button>
                            </div>
                        </div>
                    </div>
